...
def calculate_conservation_rate(dna_sequences):
    ...
    # Transpose the sequences to compare each position
    ...
    for codon_positions in codons:
        ...
        for codon in codon_positions:
            if len(codon) == 3:  # Ensure it's a valid codon
                first_two_count[codon[:2]] += 1
                third_count[codon[2]] += 1

        # Calculate conservation rate for first two and third nucleotide
